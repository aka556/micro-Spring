...
import org.xiaoyu.micro.web.controller.ControllerConfiguration;
import org.xiaoyu.micro.io.PropertyResolver;
import org.xiaoyu.micro.web.controller.ApiController.SigninObj;
import org.xiaoyu.micro.web.utils.JsonUtils;
...
public class DispatcherServletTest {
    ...
    void getHello() throws ServletException, IOException {
        var req = createMockRequest("GET", "/hello/xiaoyu", null, null);
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(200, resp.getStatus());
        assertEquals("Hello, xiaoyu", resp.getContentAsString());
    }

    @Test
    void getApiHello() throws ServletException, IOException {
        var req = createMockRequest("GET", "/api/hello/xiaoyu", null, null);
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(200, resp.getStatus());
        assertEquals("application/json", resp.getContentType());
        assertEquals("{\"name\":\"xiaoyu\"}", resp.getContentAsString());
    }

    @Test
    void getGreeting() throws ServletException, IOException {
        var req = createMockRequest("GET", "/greeting", null, Map.of("name", "xiaoyu"));
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(200, resp.getStatus());
        assertEquals("Hello, xiaoyu", resp.getContentAsString());
    ...
    void getApiGreeting() throws ServletException, IOException {
        var req = createMockRequest("GET", "/api/greeting", null, Map.of("name", "xiaoyu"));
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(200, resp.getStatus());
        assertEquals("application/json", resp.getContentType());
        assertEquals("{\"action\":{\"name\":\"xiaoyu\"}}", resp.getContentAsString());
    }

    @Test
    void getGreeting2() throws ServletException, IOException {
        var req = createMockRequest("GET", "/greeting", null, Map.of("action", "Morning", "name", "xiaoyu"));
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(200, resp.getStatus());
        assertEquals("Morning, xiaoyu", resp.getContentAsString());
    }
    ...
    void getProduct() throws ServletException, IOException {
        var req = createMockRequest("GET", "/product/123", null, Map.of("name", "xiaoyu"));
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(200, resp.getStatus());
        assertTrue(resp.getContentAsString().contains("<h1>Hello, xiaoyu</h1>"));
        assertTrue(resp.getContentAsString().contains("<a href=\"/product/123\">Micro Software</a>"));
    }

    @Test
    void postSignin() throws ServletException, IOException {
        var req = createMockRequest("POST", "/signin", null, Map.of("name", "xiaoyu", "password", "hello123"));
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(302, resp.getStatus());
        assertEquals("/home?name=xiaoyu", resp.getRedirectedUrl());
    }

    @Test
    void postRegister() throws ServletException, IOException {
        var req = createMockRequest("POST", "/register", null, Map.of("name", "xiaoyu", "password", "hello123"));
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(200, resp.getStatus());
        assertTrue(resp.getContentAsString().contains("<h1>Welcome, xiaoyu</h1>"));
    }
    ...
    void postApiRegister() throws ServletException, IOException {
        ...
        assertEquals(200, resp.getStatus());
        assertEquals("application/json", resp.getContentType());
        assertEquals("[\"xiaoyu\",true,12345]", resp.getContentAsString());
    }

    @Test
    void postSignout() throws ServletException, IOException {
        var req = createMockRequest("POST", "/signout", null, Map.of("name", "xiaoyu"));
        var resp = createMockResponse();
        this.dispatcherServlet.service(req, resp);
        assertEquals(302, resp.getStatus());
        assertEquals("/signin?name=xiaoyu", resp.getRedirectedUrl());
