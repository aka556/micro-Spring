...
public class MvcController {
    ...
    void downloadPart(HttpServletRequest request, HttpServletResponse response) throws IOException {
        response.setStatus(206);
        response.setHeader("Range", "bytes=100-108");
    ...
    ModelAndView product(@PathVariable("id") long id, @RequestParam("name") String name) {
        return new ModelAndView("/product.html", Map.of("name", name, "product", Map.of("id", id, "name", "Micro Software")));
    }

    @PostMapping("/signin")
    ModelAndView siginin(@RequestParam("name") String name, @RequestParam("password") String password) {
