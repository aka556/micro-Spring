...
import org.xiaoyu.micro.exception.*;
import org.xiaoyu.micro.io.PropertyResolver;
import org.xiaoyu.micro.io.ResourceResolver;
import org.xiaoyu.micro.web.utils.ClassUtils;
...
public class AnnotationConfigApplicationContext {
    ...
    /**
     * 创建一个Bean, 不进行字段和方法级别的注入。如果创建的bean不是Configuration bean,
     * 则在构造方法中进行依赖注入。
     */
    public Object createBeanAsEarlySingleton(BeanDefinition def) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            final Annotation[] paramAnno = parameterAnnotations[i];
            final Value value = ClassUtils.getAnnotation(paramAnno, Value.class);
            final Autowired autowired = ClassUtils.getAnnotation(paramAnno, Autowired.class);
            ...
            if (value != null) {
                ...
                // 依赖的BeanDefinition
                BeanDefinition depDef = name.isEmpty() ? findBeanDefinition(type) : findBeanDefinition(name, type);
                // 检测required==true?
                if (required && depDef == null) {
                ...
                if (depDef != null) {
                    // 获取依赖Bean:
                    Object autowiredBeanInstance = depDef.getInstance();
                    if (autowiredBeanInstance == null && !isConfiguration) {
                        // 当前依赖Bean尚未初始化，递归调用初始化该依赖Bean:
                        autowiredBeanInstance = createBeanAsEarlySingleton(depDef);
                    }
    ...
    * <code>
    ...
    protected Set<String> scanForClassNames(Class<?> configClass) {
        ...
        if (importConfig != null) {
            for (Class<?> importCnfigClass : importConfig.value()) {
                String importClassName = importCnfigClass.getName();
    ...
    // 根据name查找bean, 没有返回null
    @Nullable
    public BeanDefinition findBeanDefinition(String name) {
        return this.beans.get(name);
    }

    // 根据type和name查找bean, 如果name不存在则返回null，Name存在但是type不匹配则抛出BeanDefinitionException
    @Nullable
    ...
    // 根据type查找某个BeanDefinition, 如果不存在返回null, 如果存在多个返回@Primary标注的一个，如果有多个@Primary标注，或没有@Primary标注但找到多个，
