...
public class AnnotationConfigApplicationContext {
    ...
    public AnnotationConfigApplicationContext(Class<?> configClass, PropertyResolver propertyResolver) {
        ...
        List<BeanPostProcessor> processors = this.beans.values().stream()
                ...
                .filter(def -> def.getBeanClass().isAssignableFrom(BeanPostProcessor.class))
                .sorted().map(def -> {
    ...
    public Object createBeanAsEarlySingleton(BeanDefinition def) {
        ...
        final Annotation[][] parameterAnno = createFn.getParameterAnnotations();
        Object[] args = new Object[parameters.length];
        for (int i = 0; i < parameters.length; i++) {
            final Parameter param = parameters[i];
            final Annotation[] paramAnno = parameterAnno[i];
     ...
     */
    Map<String, BeanDefinition> createBeanDefinitions(Set<String> beanNameSet) {
        Map<String, BeanDefinition> defs = new HashMap<>();
        for (String className : beanNameSet) {
            // 获取class
            Class<?> clazz = null;
            try {
    ...
    /**
     * 根据Name和Type查找BeanDefinition, 如果Name不存在，返回null, 如果Name存在，但是type不存在，抛出异常.
     */
    @Nullable
    ...
    /**
     * 根据Type查找BeanDefinition, 如果没有找到，则返回null.
     */
    @Nullable
