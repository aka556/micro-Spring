...
import org.xiaoyu.micro.exception.BeanDefinitionException;
import org.xiaoyu.micro.exception.BeanNotOfRequiredTypeException;
import org.xiaoyu.micro.exception.NoUniqueBeanDefinitionException;
...
public class AnnotationConfigApplicationContext {
    ...
    // 根据扫描的BeanName创建BeanDefinition
    Map<String, BeanDefinition> createBeanDefinitions(Set<String> classNameSet) {
        Map<String, BeanDefinition> defs = new HashMap<String, BeanDefinition>();
        for (String className : classNameSet) {
            ...
            if (clazz.isInterface() || clazz.isAnnotation() || clazz.isEnum() || clazz.isRecord()) {
                continue;
            }
            // 是否标注了@Component注解
        ...
        for (String pkg : scanPackages) {
            ...
            if (logger.isDebugEnabled()) {
                classList.forEach((className -> {
                    logger.atDebug().log("class found by component scan: {}", className);
                }));
            }
    ...
    /**
     * 根据name查找BeanDefinition, 如果不存在则返回null
     */
    @Nullable
